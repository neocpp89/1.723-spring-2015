...
\end{align}
This is clearly greater than one (unless $u = 0$, in which case it is equal to one), so the method is unconditionally unstable.

\section{Problem 5}
\subsection{Part 1}
As in the previous homework, we first note that we can use the chain rule in combination with the definitions to nondimensionalize the equations.
